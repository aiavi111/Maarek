...
export default function CheckoutPage() {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    setTimeout(() => {
      setPlaced({
        orderNo: `MR-${Math.floor(1000 + Math.random() * 9000)}`,
        total: totals.total,
      });
      clearCart();
    }, 1500);
      ...
      <div className="sticky top-0 z-30 border-b border-line bg-base/85 backdrop-blur-2xl pt-safe">
        <div className="relative mx-auto flex h-14 max-w-[640px] items-center justify-center px-5">
          <motion.button
            ...
            onClick={() => router.back()}
            className="absolute left-4 grid size-10 place-items-center rounded-full border border-line bg-card cursor-pointer"
          >
            <ChevronLeft size={19} />
          </motion.button>
        ...
        >
          ...
          <AnimatePresence mode="wait" initial={false}>
            {mode === "delivery" ? (
              ...
              >
                <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-accent text-onfg">
                  <MapPin size={20} />
                </span>
                <div>
                    ...
                    <AnimatePresence>
                      {active && (
                        <motion.span
                          ...
                          className="absolute right-3 top-3 grid size-5 place-items-center rounded-full bg-accent"
                        >
    ...
    >
      <div className="relative mb-8">
        ...
        <svg viewBox="0 0 120 120" className="size-28">
          <circle cx="60" cy="60" r="54" fill="none" stroke="#f5efe01a" strokeWidth="3" />
          <motion.circle
            cx="60"
            cy="60"
            r="54"
...
function inputCls(hasError: boolean) {
  return cn(
    "h-12 w-full rounded-2xl border bg-card px-4 text-[15px] font-medium placeholder:text-dim focus:outline-none transition-colors",
    hasError ? "border-danger/60" : "border-line focus:border-accent/50",
  );
}
